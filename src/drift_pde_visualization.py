...
def _dataset_tag(dataset_dir: str) -> str:
    ds = os.path.basename(os.path.normpath(str(dataset_dir))).lower()
    if "exid" in ds:
        return "exiD"
    if ds == "ind" or "ind" in ds:
        return "inD"
    if "round" in ds:
        return "rounD"
    return os.path.basename(os.path.normpath(str(dataset_dir)))


DATASET_DIR    = _env_str("DREAM_DATASET_DIR", r"c:\field_modeling\data\inD")
RECORDING_ID   = _env_str("DREAM_RECORDING_ID", "00")            # zero-padded recording number, e.g. "00", "26"
EGO_TRACK_ID   = _env_int("DREAM_EGO_TRACK_ID", 45)              # set env var to "none" for auto-selection
EGO_LANE       = _env_int("DREAM_EGO_LANE", 1)                   # Lane the ego starts on (0=left, 1=centre, 2=right)
N_LANES        = _env_int("DREAM_N_LANES", 3)                    # number of lanes to fit
LANE_WIDTH     = float(_env_str("DREAM_LANE_WIDTH", "3.75"))     # m used for parallel-path fallback
EGO_MIN_FRAMES = _env_int("DREAM_EGO_MIN_FRAMES", 80)            # minimum track length (frames) for ego candidate

dt             = float(_env_str("DREAM_DT", "0.1"))              # simulation step [s]
N_t            = _env_int("DREAM_NT", 400)                       # number of simulation steps (20 s)
WARMUP_S       = float(_env_str("DREAM_WARMUP_S", "5.0"))        # DRIFT warm-up duration [s]

INTEGRATION_MODE  = _env_str("DREAM_INTEGRATION_MODE", "conservative")
DRIFT_CELL_M      = float(_env_str("DREAM_DRIFT_CELL_M", "2.0")) # DRIFT grid cell size [m]
SCENE_MARGIN      = float(_env_str("DREAM_SCENE_MARGIN", "60.0"))# margin beyond track bbox [m]
SCENARIO_MODE     = _env_str("DREAM_SCENARIO_MODE", "drift_overlay")   # "drift_overlay" or "benchmark"
SOURCE_MODE       = _env_str("DREAM_SOURCE_MODE", "all")         # "gvf" | "ada" | "apf" | "both" | "all"
...
_dataset_name = _dataset_tag(DATASET_DIR)
_default_save_dir = (
    f"figsave_DRIFT_dataset_{_dataset_name}_{RECORDING_ID}{_src_tag}"
    if SCENARIO_MODE == "drift_overlay"
    else f"figsave_DREAM_dataset_{_dataset_name}_{RECORDING_ID}"
)
save_dir = os.path.join(
    os.path.abspath(os.path.dirname(__file__)),
    _env_str("DREAM_SAVE_DIR", _default_save_dir),
)
...
def select_ego_track(tracks, tracks_meta, class_map, min_frames, ego_lane,
    ...
    candidates = []
    for i_tr, tm in enumerate(tracks_meta):
        tid = tm["trackId"]
...
def fit_lane_paths(tracks, tracks_meta, class_map, ego_track_id,
    ...
    """
    _ego_idx_lf = next((i for i, tm in enumerate(tracks_meta) if tm["trackId"] == ego_track_id), None)
    if _ego_idx_lf is None:
        raise ValueError(f"trackId {ego_track_id} not found in tracksMeta")
    ego_tr  = tracks[_ego_idx_lf]
    ...
    if len(pts) < n_lanes * 10:
        ...
        return _parallel_fallback(tracks[_ego_idx_lf], n_lanes)
...
def _bind_dataset_path_adapters_to_utils(utils_obj, tag):
    ...
    DecisionMaking.util methods call module-level get_path_info(). Without
    rebinding they keep using Path.path highway references.
    """
...
def draw_frame_drift_overlay(i, frame_idx, tracks, tracks_meta, class_map,
    ...
    if EGO_TRACK_ID is not None:
        ego_tm = tracks_meta[_ego_track_idx]
        ego_tr = tracks[_ego_track_idx]
...
_was_auto_select = EGO_TRACK_ID is None
if _was_auto_select:
...
_ego_track_idx = next((i for i, tm in enumerate(tracks_meta) if tm["trackId"] == EGO_TRACK_ID), None)
if _ego_track_idx is None:
    raise ValueError(
        f"trackId {EGO_TRACK_ID} not found in {DATASET_DIR}/{rec}_tracksMeta.csv "
        f"(available trackId range: [{tracks_meta[0]['trackId']}, {tracks_meta[-1]['trackId']}])"
    )
print(f"  {'Auto-selected' if _was_auto_select else 'User-specified'} ego: "
      f"trackId={EGO_TRACK_ID}  class={class_map.get(EGO_TRACK_ID)}  "
      f"frames={tracks_meta[_ego_track_idx]['numFrames']}")

ego_meta  = tracks_meta[_ego_track_idx]
ego_track = tracks[_ego_track_idx]
